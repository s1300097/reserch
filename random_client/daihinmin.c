...
int allGroup(int candidates[10][8][15], int my_cards[8][15], int group[8][15])
{
	int i, j, k, count = 0, qty, found;

	for (i = 0; i < 10; i++)
	{
		clearTable(candidates[i]);
	}

	for (j = 1; j < 14; j++)
	{
		if (group[0][j] > 1 || group[1][j] > 1 || group[2][j] > 1 || group[3][j] > 1)
		{
			found = 0;
			for (k = 0; k < 4; k++)
			{
				...
				{
					qty = group[k][j];
					candidates[count][k][j] = 1;
					found++;
				}
			}
			for( k = 0; found < qty; k++)
			{
				if(my_cards[k][j] == 0 && (state.lock == 0 || state.suit[k] == 1))
				{
					candidates[count][k][j] = 2;
					found++;
				}
			}
			count++;
...
int allSequence(int candidates[20][8][15], int my_cards[8][15], int sequence[8][15])
{
		...
		for (i = 1; i < 15; i++)
		{
			if( sequence[j][i] > 2)
			{
				...
				for( k = 0; k < value; k++)
				{
					if( my_cards[j][i + k] > 0)
					{
						candidates[count][j][i + k] = 1;
					}
					else
					{
						candidates[count][j][i + k] = 2;
					}
				}
				count++;
			}
...
int allSolo(int candidates[20][8][15], int my_cards[8][15], int joker_flag)
{
	int i, j, count = 0;
	int memo[20];
	char suit_char[5] = {'S','H','D','C','J'};
	...
	{ // 低い方からさがし
		...
		{	
			if (my_cards[i][j] == 1)
			{				  // カードを見つけたら
				candidates[count][i][j] = my_cards[i][j]; // out_cardsにのせ,ループを抜ける。
				memo[count] = i + j* 10;
				count++;
			}
		}
...
void lead(int out_cards[8][15], int my_cards[8][15])
{
	...
	int temp[20][8][15];	  // 一時使用用のテーブル
	int candidates[50][8][15]; // 候補手札を格納するテーブル
	int num_candidates = 0; // 候補手札の数
	int i, seq_c, group_c, solo_c;

	srand((unsigned int)time(NULL));
	clearTable(group);
	clearTable(sequence);
	clearTable(out_cards);
	clearTable(candidates[0]); // パス用の候補手札を用意しておく
	...
	if(qtyOfCards(sequence) > 0)
	{	
		seq_c = allSequence(temp, my_cards, sequence);
		for(i = 0; i < seq_c; i++)
		{	
			copyTable(candidates[num_candidates++], temp[i]);
		}
	}
	
	if(qtyOfCards(group) > 0)
	{
		group_c = allGroup(temp, my_cards, group);
		for(i = 0; i < group_c; i++)
		{	
			copyTable(candidates[num_candidates++], temp[i]);
		}
	...
	solo_c = allSolo(temp, my_cards, state.joker);
	for(i = 0; i < solo_c; i++)
	{	
		copyTable(candidates[num_candidates++], temp[i]);
	}
	...
	if(g_logging == 1)
	{
		printf("lead candidates \n");
		for(i=0;i<num_candidates;i++)
		{
			printf("candidate %d\n",i);
			outputTable(candidates[i]);
		}
	}
	
	int chosen = rand() % num_candidates; // 候補手札の中からランダムに選ぶ
	copyTable(out_cards, candidates[chosen]);
}
...
void followSolo(int out_cards[8][15], int my_cards[8][15], int joker_flag)
{
	...
	*/
	...
	int candidates[20][8][15]; // 候補手札を格納するテーブル
	int num_candidates; // 候補手札の数
	int i;
	...
	num_candidates = allSolo(candidates, temp, joker_flag);

	if(g_logging == 1)
	{
		printf("follow solo candidates \n");
		for(i = 0;i<num_candidates;i++)
		{
			printf("candidate %d\n",i);
			outputTable(candidates[i]);
...
void followGroup(int out_cards[8][15], int my_cards[8][15], int joker_flag)
{
	...
	*/
	...
	if(state.joker == 1)
	{
		makeJGroupTable(group, temp); // ジョーカーを使って枚数組を書き出す
	}
	else
	{
		makeGroupTable(group, temp); // 枚数組を書き出す
	}
	nCards(ngroup, group, state.qty); // 出すべき枚数組だけを抽出
	num_candidates = allGroup(candidates, temp, ngroup);
	if (num_candidates == 0)
	{
		if(g_logging == 1)
		{
			printf("no follow group candidates \n\n");
		}
		return;
	}
	num_candidates++;
	...
	if(g_logging == 1)
	{
		printf("follow group candidates \n");
		for(i=0;i<num_candidates;i++)
		{
			printf("candidate %d\n",i);
			outputTable(candidates[i]);
...
void followSequence(int out_cards[8][15], int my_cards[8][15], int joker_flag)
{
	...
	*/
	...
	if(state.joker == 1)
	{
		makeJKaidanTable(seq, temp); // ジョーカーを使って階段を書き出す
	}
	else
	{
		makeKaidanTable(seq, temp); // 階段を書き出す
	}
	nCards(nseq, seq, state.qty); // 出すべき階段だけを抽出
	num_candidates = allSequence(candidates, temp, nseq);
	if (num_candidates == 0)
	{
		if(g_logging == 1)
		{
			printf("no follow sequence candidates \n\n");
		}
		return;
	}
	num_candidates++;
	...
	if(g_logging == 1)
	{
		printf("follow sequence candidates \n");
		for(i=0;i<num_candidates;i++)
		{
			printf("candidate %d\n",i);
			outputTable(candidates[i]);
...
void followGroupRev(int out_cards[8][15], int my_cards[8][15], int joker_flag)
{
	...
	*/
	...
	if (num_candidates == 0)
	{
		if(g_logging == 1)
		{
			printf("no follow group rev candidates \n\n");
		}
		return;
	}
	...
	if(g_logging == 1)
	{
		printf("follow group rev candidates \n");
		for(i=0;i<num_candidates;i++)
		{
			printf("candidate %d\n",i);
			outputTable(candidates[i]);
...
void followSequenceRev(int out_cards[8][15], int my_cards[8][15], int joker_flag)
{
	...
	*/
	...
	if (num_candidates == 0)
	{
		if(g_logging == 1)
		{
			printf("no follow sequence rev candidates \n");
		}
		return;
	}
	num_candidates++;
	if(g_logging == 1)
	{
		printf("follow sequence rev candidates \n");
		for(i=0;i<num_candidates;i++)
		{
			printf("candidate %d\n",i);
			outputTable(candidates[i]);
